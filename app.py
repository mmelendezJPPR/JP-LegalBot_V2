...
def es_saludo(mensaje: str) -> bool:
    """Detectar si el mensaje es un saludo o presentación"""
    mensaje_lower = mensaje.lower().strip()
    
    # Saludos comunes
    saludos = [
        'hola', 'hi', 'hello', 'buenos días', 'buenas tardes', 'buenas noches',
        'buen día', 'buena tarde', 'buena noche', 'saludos', 'que tal',
        'qué tal', 'como estas', 'cómo estás', 'como está', 'cómo está'
    ]
    
    # Presentaciones
    presentaciones = [
        'soy', 'me llamo', 'mi nombre es', 'quien eres', 'quién eres',
        'que eres', 'qué eres', 'presentate', 'preséntate'
    ]
    
    # Verificar si es solo un saludo corto o contiene palabras de saludo/presentación
    if len(mensaje_lower) <= 20:  # Mensajes cortos probablemente son saludos
    ...
    for presentacion in presentaciones:
        if presentacion in mensaje_lower:
            return True
...
def generar_mensaje_bienvenida() -> Dict:
    ...
    mensaje_bienvenida = """
¡Hola! Soy JP-LegalBot, tu asistente para consultas sobre el Reglamento Conjunto de Emergencia JP-RP-41.
